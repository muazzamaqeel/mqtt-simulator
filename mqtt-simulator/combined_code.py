...
class Topic(threading.Thread):
    ...
    def run(self):
        self.connect()  # Initial connection
        while self.loop:
            try:
            ...
            except Exception as e:  # Catch all exceptions to handle reconnection
    ...
    def generate_protobuf_payload(self):
        # Create a SensorData Protobuf message dynamically
        ...
        # Dynamically assign each sensor data to the data_map in Protobuf
        for sensor_config in self.topic_data:
            field_name = sensor_config.name
            field_value = self.generate_value(sensor_config)

            # Check if the field exists in the Protobuf schema
            if hasattr(sensor_data, 'data_map'):
                # Set the field using reflection
                sensor_data.data_map[field_name] = field_value
                data_dict[field_name] = field_value  # Also store it in the dictionary for logging

        # Store the data dictionary as payload for logging purposes
        self.payload = data_dict

        # Return the serialized Protobuf message
        return sensor_data.SerializeToString()

    def generate_value(self, config):
        if config.is_int:
            return random.randint(config.data['MIN_VALUE'], config.data['MAX_VALUE']).to_bytes(4, byteorder='little')
        else:
            float_val = random.uniform(config.data['MIN_VALUE'], config.data['MAX_VALUE'])
            return bytes(struct.pack("f", float_val))  # Convert to bytes instead of bytearray
    ...
    def extract_sensor_group(self, topic_url):
        return "default_group"  # Or customize based on your requirements
